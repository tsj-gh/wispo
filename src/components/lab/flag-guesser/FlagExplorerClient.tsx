...
export function FlagExplorerClient() {
  ...
  const filtered = useMemo(() => {
    ...
    if (colorFilter.length > 0) list = list.filter((r) => colorFilter.some((c) => r.colors.includes(c)));
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1400px] flex-col px-4 py-4 md:py-6">
      <GamePageHeader
        ...
        titleJa="国旗エクスプローラー"
        breadcrumbs={[{ label: "ホーム", href: "/" }, { label: "教材一覧", href: "/#lab-cards" }, { label: "フラッグゲッサー", href: "/lab/flag-guesser" }, { label: "エクスプローラー", href: "/lab/flag-guesser/explorer" }]}
      />
        ...
        <>
          <section className="mb-6 space-y-4 rounded-2xl border border-[color-mix(in_srgb,var(--color-text)_10%,transparent)] bg-[color-mix(in_srgb,var(--color-text)_5%,transparent)] p-4 backdrop-blur sm:p-5">
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              ...
              <label className="flex flex-col gap-1.5 text-xs"><span>サブリージョン</span><select value={subRegion} disabled={!region} onChange={(e) => { setSubRegion(e.target.value); setIntermediateRegion(""); }} className="rounded-lg border px-3 py-2 text-sm"><option value="">（すべて）</option>{subRegionOptions.map((s) => <option key={s} value={s}>{s}</option>)}</select></label>
              <label className="flex flex-col gap-1.5 text-xs"><span>中間リージョン</span><select value={intermediateRegion} disabled={!intermediateOptions.length} onChange={(e) => setIntermediateRegion(e.target.value)} className="rounded-lg border px-3 py-2 text-sm"><option value="">（すべて）</option>{intermediateOptions.map((s) => <option key={s} value={s}>{s}</option>)}</select></label>
