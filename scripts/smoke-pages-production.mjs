/**
 * 本番（またはプレビュー）URL の HTML が参照する JS/CSS と、
 * フラッグエクスプローラー等が fetch する静的 JSON が HTTP 200 か検証する。
...
async function assetOk(assetPath) {
  ...
  if (buf.byteLength < MIN_DATA_ASSET_BYTES) {
  ...
  if (assetPath.endsWith(".json")) {
    try {
      ...
      return { assetPath, status: res.status, ok: false, reason: "invalid JSON" };
    }
  }
...
async function runOnce() {
  const allAssets = new Set(CRITICAL_DATA_ASSETS);
  ...
  return { allAssets, bad, dataAssetCount: CRITICAL_DATA_ASSETS.length };
}
...
async function main() {
  ...
  for (let attempt = 1; attempt <= RETRIES; attempt++) {
    const { allAssets, bad, dataAssetCount } = await runOnce();
    if (!bad.length) {
      console.log(
        `[smoke-pages-production] OK — ${PAGES.length} page(s), ${allAssets.size} asset(s)` +
          ` (incl. ${dataAssetCount} data JSON) on ${SITE_URL}` +
